// pages/mine/address/update/update.js
const app = getApp();
...
Page({
  ...
  submitData: function () {
    ...
    if (this.verify()) {
      wx.request({
        url: apiUrl + '/Api/Member/editAddress',
        data: {
          id: that.data.id,
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          wx.request({
            url: apiUrl + '/Api/Member/delAddress?id=' + that.data.id,
            header: {
  ...
  getAddressData: function() {
    ...
    let that = this
    wx.request({
      url: apiUrl + '/Api/Member/getAddressInfo?address_id=' + this.data.id,
      header: {
        'content-type': 'application/json',
        'Cookie': 'PHPSESSID=' + wx.getStorageSync("sessionID")
      },
      ...
      success: function (res) {
        ...
        if (res.data.status == '1') {
          ...
          that.setData({
            ...
            siteName: res.data.data.district_str,
            switch1Checked: ism
          })
        } else {
          wx.showToast({
